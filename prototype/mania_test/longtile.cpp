...
LongTile::LongTile(QPointF start_point, QPointF end_point, QPointF delta,TileType t_type, int length, QObject *parent):
    NewTile(start_point, end_point, delta, Long, parent), tile_type(t_type), length(length)
{
...
void LongTile::init(int width){
    ...
    tile_map = tile_map.scaled(width,pos_point.y(), Qt::IgnoreAspectRatio);
